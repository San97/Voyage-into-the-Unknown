...
from helper.for_new_maze.heuristics.generate_euclidean_for_new_maze import cal_euclidean_for_new_maze
from src.constants import INF


def set_attr_new_maze_euclidean(Maze, Dim):
    maze = Maze
    dim = Dim
    ...
    for i in range(0, dim):
        for j in range(0, dim):
            if (i == 0) and (j == 0):
                ...
                maze[i][j].hn = cal_euclidean_for_new_maze(i, j, dim)  # n-dim -> n+n
                ...
                maze[i][j].hn = cal_euclidean_for_new_maze(i, j, dim)
                maze[i][j].gn = cal_gn_for_new_maze(i, j, dim, maze)
                maze[i][j].fn = INF
                maze[i][j].parent = (-1, -1)
