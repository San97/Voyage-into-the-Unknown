"""Test to compare euclidean and Weighted euclidean and plots for both Density vs AVG Trajectory Length and
...
from src.Q9.weighted_euclidean.repeated_astar_for_q9_weighted_euclidean import repeated_forward_astar_weighted
from src.Q9.weighted_euclidean.repeated_astar_for_q9_euclidean import repeated_forward_astar_euclidean
import numpy as np
...
print("For weighted euclidean(inadmissible):")
for p in Prob:
...
print("\n\n")
print("Now for euclidean(admissible)")
...
for p in Prob:
    ...
    for x in range(0, iterations_per_prob):
        maze = create_maze(dim)
        set_attr_euclidean(maze, dim, p)
        new_path_from_repeated_forward_astar, cells_processed = repeated_forward_astar_euclidean(maze, dim, 0, 0)
